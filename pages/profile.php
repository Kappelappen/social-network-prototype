<?php

declare(strict_types=1);
require_once "../private/application/engine.php";

?>

<?php require_once ("../private/includes/header_inc.php"); ?>

<body>

<div class="page-headline">
<?php require_once ("../private/includes/headline_inc.php"); ?>
</div>

<div class="page-menubar">
<?php require_once ("../private/includes/main_nav_inc.php"); ?>
</div>

<div class="main-view">

<?php

    $status = $account->getProfileVisibility();
    if ($status === 'public' || $status === "") {

?>

    <?php require_once ("../private/includes/profile_inc.php"); ?>


<?php } ?>

<?php if ($status === 'private') { ?>

    <?php require_once ("../private/includes/private_inc.php"); ?>

<?php } ?>

<?php if ($status === 'friends') { ?>

    <?php require_once ("../private/includes/only_friends_inc.php"); ?>

<?php } ?>

</div>

<div class="page-footer">
<?php require_once ("../private/includes/footer_inc.php"); ?>
</div>

</body>
</html>